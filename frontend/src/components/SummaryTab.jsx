...
const GRADE_SIGNAL = {
  ...
  'B':  { label: 'UNDERPERFORM',color: '#E88D14' },
...
export default function SummaryTab() {
  ...
  const signal = GRADE_SIGNAL[realtime?.grade] ?? { label: 'N/A', color: '#555' };
  ...
  const SectionHeader = ({ title, subTitle, question, description, color }) => (
    <div style={{ marginBottom: '15px', borderTop: `2px solid ${color}`, paddingTop: '16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px', flexWrap: 'wrap' }}>
        ...
        <span style={{ color: '#555', fontSize: '12px' }}>({subTitle})</span>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '25px', padding: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '24px', alignItems: 'start' }}>
        ...
        {/* 좌측: 주요 신호 요약 */}
        <div style={{ padding: '24px', backgroundColor: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              ...
              { label: 'Quant Score',        value: realtime?.score != null ? Number(realtime.score).toFixed(1) : '—', color: '#e8e8e8' },
              { label: 'L1 (Quant)',         value: realtime?.l1 ?? '—',                              color: '#D85604' },
              { label: 'L2 (NLP/AI)',        value: realtime?.l2 ?? '—',                              color: '#7c3aed' },
              { label: 'L3 (Market)',        value: realtime?.l3 ?? '—',                              color: '#0891b2' },
              { label: 'Strong Buy Signal',  value: realtime?.strong_buy_signal  ? '✅ YES' : '—',   color: '#00F5FF' },
              { label: 'Strong Sell Signal', value: realtime?.strong_sell_signal ? '🔴 YES' : '—',   color: '#AD1B02' },
            ].map(row => (
        ...
        <div style={{ padding: '24px', backgroundColor: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: '16px' }}>
          ...
          {!ratingHistory ? (
            <div style={{ color: '#555', fontSize: 12, textAlign: 'center', padding: 20 }}>Loading...</div>
          ) : (
                  ...
                  <div key={idx} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        ...
                        {item.score != null && (
                          <div style={{ color: '#555', fontSize: 10, fontFamily: 'sans-serif' }}>
                            Score {typeof item.score === 'number' ? item.score.toFixed(1) : item.score}
                          </div>
                        )}
                      </div>
                    </div>
                    <div style={{ color: '#555', fontSize: 11, fontFamily: 'sans-serif' }}>{item.date}</div>
                  </div>
                );
              })}
            </div>
...
function SignalBanner({ signal, realtime }) {
  ...
  const layers = [
    { key: 'L1', label: 'Quant',  score: realtime.l1, color: '#D85604' },
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        <div>
          <div style={{ fontSize: 10, color: '#555', fontFamily: 'sans-serif', letterSpacing: 1.5, marginBottom: 4 }}>QUANT AI · FINAL SIGNAL</div>
        ...
        {realtime.score != null && (
          <div style={{ textAlign: 'center', padding: '8px 14px', background: `${signal.color}15`, borderRadius: 8, border: `1px solid ${signal.color}30` }}>
            ...
            <div style={{ fontSize: 9, color: '#555', letterSpacing: 1 }}>SCORE</div>
          </div>
        )}
      </div>
      <div style={{ display: 'flex', gap: 10 }}>
        {layers.map(l => (
          <div key={l.key} style={{
            ...
            <div style={{ fontSize: 9, color: '#555', fontFamily: 'sans-serif', letterSpacing: 1, marginBottom: 4 }}>{l.key}</div>
...
//                       {item.score && <div style={{ color: '#555', fontSize: '10px', fontFamily: 'sans-serif' }}>Score {item.score.toFixed ? item.score.toFixed(1) : item.score}</div>}
//                     </div>
//                   </div>
//                   <div style={{ color: '#555', fontSize: '11px', fontFamily: 'sans-serif' }}>{item.date}</div>
//                 </div>
//               );
//             })}
//           </div>
