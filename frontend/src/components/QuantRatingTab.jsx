...
import { C, FONT, gradeColor, chgColor } from '../styles/tokens';
...
const getRating = (score) => {
  ...
  if (score >= 80) return { label: 'S',  title: 'Strong Buy',  color: C.gradeS, status: '기관급 QVM 팩터 최상단 정렬',      action: '퀀트 엔진이 포착한 최상위 5% 종목입니다. 구조적 성장과 가격 모멘텀이 결합된 슈퍼 사이클 구간입니다.' };
  if (score >= 72) return { label: 'A+', title: 'Buy',         color: C.gradeS, status: '펀더멘털 가속화 + 기관 수급 유입',  action: '확실한 이익 개선과 함께 시장 대비 초과 수익을 창출할 준비가 끝난 종목입니다.' };
  if (score >= 65) return { label: 'A',  title: 'Outperform',  color: C.gradeA, status: '안정적 밸류에이션 + 추세 강화',    action: '시장 지수를 상회할 체력이 충분합니다. 조정 시마다 비중을 늘려가는 전략이 권장됩니다.' };
  if (score >= 55) return { label: 'B+', title: 'Hold',        color: C.gradeBP, status: '팩터간 불균형 또는 에너지 응축',   action: '기업의 질은 우수하나 가격 반영이 다소 과하거나 모멘텀이 부족한 관망 구간입니다.' };
  if (score >= 45) return { label: 'B',  title: 'Underperform',color: C.gradeB, status: '성장성 정체 및 밸류 부담 발생',    action: '기대 수익률이 시장 평균을 밑돌 가능성이 높습니다. 교체 매매를 고려하십시오.' };
  if (score >= 35) return { label: 'C',  title: 'Sell',        color: C.gradeB, status: '펀더멘털 훼손 및 하락 추세',       action: '자산 보호를 위해 비중 축소 및 리스크 관리가 시급합니다.' };
  return               { label: 'D',  title: 'Strong Sell', color: C.gradeD, status: '재무적 리스크 및 역성장 트랩',      action: '퀀트 지표 상 최하위 구간입니다. 즉시 관심을 끄십시오.' };
};
...
export default function QuantRatingTab() {
  ...
  if (loading) return (
    <div style={{ color: C.gradeA, textAlign: 'center', padding: '100px', fontWeight: 900, fontFamily: 'monospace' }}>
      ANALYZING...
    </div>
  );

  if (!data) return (
    <div style={{ color: C.gradeB, textAlign: 'center', padding: '80px', fontFamily: 'monospace' }}>
      데이터를 불러올 수 없습니다.
    </div>
  );
  ...
  return (
    <div style={{ backgroundColor: '#000', color: '#fff', padding: '20px', fontFamily: 'Inter, sans-serif' }}>
      ...
      {!hasBatchScore && (
        <div style={{
          background: C.bgDeeper, border: '1px solid #E88D1440',
      ...
      {/* ── 전략 가이드 (아코디언) */}
      <div style={{ marginBottom: 30, backgroundColor: C.bgDeeper, borderRadius: 24, border: '1px solid #1A1A1A', overflow: 'hidden', position: 'relative' }}>
        <div style={{ position: 'absolute', right: -10, bottom: -20, fontSize: 120, fontWeight: 900, color: C.surfaceAlt, zIndex: 0, userSelect: 'none' }}>4-Factor</div>
      ...
      <div style={{ display: 'flex', gap: 20, marginBottom: 40, flexWrap: 'wrap' }}>
        ...
        {/* 등급 카드 */}
        <div style={{ flex: 1, minWidth: 320, backgroundColor: C.bgDeeper, borderRadius: 24, border: '1px solid #1A1A1A', padding: 30, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', right: 20, top: 20, fontSize: 80, fontWeight: 900, color: C.surfaceAlt, zIndex: 0 }}>{rating.label}</div>
        ...
        {/* 레이더 차트 */}
        <div style={{ flex: 1, minWidth: 320, backgroundColor: C.bgDeeper, borderRadius: 24, border: '1px solid #1A1A1A', padding: 30 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 12 }}>
            {[
              ...
              { k: 'STABILITY', v: data.stabilityWeighted, c: C.up },
            ].map(row => (
...
const SectorGroup = ({ title, subtitle, goal, weight, children }) => (
  <div style={{ width: '100%' }}>
    ...
    <div style={{ padding: '10px 0 20px', display: 'flex', flexDirection: 'column', gap: 4 }}>
      <div style={{ fontSize: 14, fontWeight: 700, color: C.textGray }}>"{subtitle}"</div>
...
const MetricRow = ({ label, value, score, sub, formula, scoring, desc }) => (
  <div style={{ display: 'flex', alignItems: 'center', backgroundColor: C.bgDeeper, padding: '18px 28px', borderBottom: '1px solid #1A1A1A' }}>
    {/* 좌: 라벨 + 설명 */}
    <div style={{ flex: '0 0 38%', paddingRight: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
        <span style={{ fontSize: 15, fontWeight: 900, color: C.gradeA }}>{label}</span>
    ...
    <div style={{ flex: '0 0 37%', borderLeft: '1px solid #222', paddingLeft: 24, display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ fontSize: 11, color: '#555' }}>
        ...
        <span style={{ color: C.gradeBP, fontWeight: 700 }}>{scoring}</span>
      </div>
    </div>
    {/* 우: 현재값 + 점수 */}
    <div style={{ flex: 1, textAlign: 'right' }}>
      ...
      {score != null && (
        <div style={{ fontSize: 10, color: C.gradeA, fontFamily: 'monospace', marginTop: 2 }}>
