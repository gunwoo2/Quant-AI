...
const HistoricalTab = () => {
  ...
  const { initialStart, initialEnd } = useMemo(() => {
    ...
    return { initialStart: startObj.toISOString().split('T')[0], initialEnd: now.toISOString().split('T')[0] };
  }, []);
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      setData(Array.isArray(res.data?.ohlcv) ? res.data.ohlcv : []);
    } catch (err) { console.error(err); setFetchError(true); } finally { setLoading(false); }
    ...
    <div style={{ width: '100%', maxWidth: '1400px', margin: '0 auto', position: 'relative' }}>
      ...
      <div style={tableWrapperStyle}>
        <table style={{ ...tableStyle, tableLayout: 'fixed' }}>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="7" style={loadingTdStyle}>LOADING...</td></tr>
            ) : data.length === 0 ? (
              <tr><td colSpan="7" style={{ ...loadingTdStyle, color: '#444' }}>데이터가 없습니다. 날짜 범위를 확인하세요.</td></tr>
            ) : data.map((item, index) => {
