...
const COLUMNS = [
  ...
  { key: "conviction_score", label: "CONV", width: 50, tip: "Conviction 확신도 (0~100)" },
  { key: "grade",  label: "GRADE",   width: 50  },
  { key: "signal", label: "SIGNAL",  width: 100 },
];
// 체크박스(28) + 컬럼합(923) + 좌우패딩(32) = 983px
...
function Row({ row, odd, checked, onCheck, onClick }) {
  ...
  return (
    ...
    >
      ...
      {/* 9.5 CONVICTION */}
      <div style={{ width: 50, minWidth: 50, flexShrink: 0, padding: "0 4px", textAlign: "center" }}>
        {row.conviction_score != null ? (
          <span style={{
            fontSize: 9, fontWeight: 700, padding: "2px 5px", borderRadius: 3,
            background: row.conviction_score >= 70 ? `${C.up}18` : row.conviction_score >= 40 ? `${C.golden}18` : `${C.down}18`,
            color: row.conviction_score >= 70 ? C.up : row.conviction_score >= 40 ? C.golden : C.down,
            border: `1px solid ${row.conviction_score >= 70 ? C.up : row.conviction_score >= 40 ? C.golden : C.down}30`,
            fontFamily: FONT.mono,
          }}>
            {Math.round(row.conviction_score)}
          </span>
        ) : (
          <span style={{ fontSize: 9, color: C.border, fontFamily: FONT.mono }}>—</span>
        )}
      </div>
