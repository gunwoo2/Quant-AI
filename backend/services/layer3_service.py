...
def _get_technical(stock_id: int, today: date) -> dict:
    ...
    # ── flat camelCase 필드 (프론트엔드 직접 참조용) ──
    flat = {
        ...
        "obvCurrent": _f(row.get("obv_current"), 0),
        "obvMa20": _f(row.get("obv_ma20"), 0),
        "obvTrend": row.get("obv_trend"),
        "obvScore": _f(row["obv_score"]),
        "volume20dAvg": _f(row.get("volume_20d_avg"), 0),
        "volumeSurgeRatio": _f(row["volume_surge_ratio"], 4),
        "volumeSurgeScore": _f(row["volume_surge_score"]),
        "goldenCross": bool(row.get("golden_cross")),
        "deathCross": bool(row.get("death_cross")),
        "ma50": _f(row.get("ma_50")),
        "ma200": _f(row.get("ma_200")),
        "goldenCrossScore": _f(row.get("golden_cross_score")),
        "bbSqueeze": bool(row.get("bb_squeeze")),
        "bbSqueezeScore": _f(row.get("bb_squeeze_score")),
        "bbWidth": _f(row.get("bb_width"), 4),
        "bbUpper": _f(row.get("bb_upper")),
        "bbLower": _f(row.get("bb_lower")),
        "ma20StreakDays": row.get("ma20_streak_days") or 0,
        "ma20StreakScore": _f(row.get("ma20_streak_score")),
        "breakout52w": bool(row.get("breakout_52w")),
        "breakout52wScore": _f(row.get("breakout_52w_score")),
    }
