...
def calc_stability_scores(price_df, eps_history: list,
    ...
    eps_history: 최근 3~5년 연간 EPS 리스트 (최신→과거).
    dividend_years: 배당 지급 연수.
    """
    import pandas as pd

    # 1. 수익 안정성 (EPS 표준편차 기반)
    # raw 값 초기화 (DB 저장용)
    annual_vol = None
    cv = None
    ...
    if eps_history and len(eps_history) >= 2:
        ...
        if len(arr) >= 2:
            cv = np.std(arr) / abs(np.mean(arr)) if np.mean(arr) != 0 else 999
            if cv < 0.15:     earnings_stab_score = 30.0
            elif cv < 0.30:   earnings_stab_score = 22.0
            elif cv < 0.50:   earnings_stab_score = 14.0
            elif cv < 0.80:   earnings_stab_score = 6.0
    ...
    total = round(earnings_stab_score + vol_score + mdd_score + div_score, 2)

    return {
        # ── 원시값 (DB 저장용) ──
        "annualized_volatility_250d": annual_vol,
        "low_vol_score":              vol_score,
        "eps_cv_3y":                  cv,
