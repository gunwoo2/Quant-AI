    ...
    with get_cursor() as cur:
        ...
        if with_label:
            ...
            LEFT JOIN LATERAL (
                ...
                WHERE stock_id = s.stock_id AND price_date >= '{fwd_date}'::date
                ORDER BY price_date ASC LIMIT 1
            ) p_fwd ON TRUE
            LEFT JOIN LATERAL (
                SELECT close_price FROM stock_prices_daily
                WHERE stock_id = s.stock_id AND price_date <= '{target_date}'::date
        ...
        cur.execute(f"""
            SELECT
                ...
                -- Layer2 sub-scores
                l2.news_score, l2.analyst_score, l2.insider_score,
                -- Layer3 sub-scores
            ...
            LEFT JOIN LATERAL (
                SELECT news_score, analyst_score, insider_score
                FROM layer2_scores
                WHERE stock_id = s.stock_id AND calc_date <= %s
                ORDER BY calc_date DESC LIMIT 1
            ) l2 ON TRUE
