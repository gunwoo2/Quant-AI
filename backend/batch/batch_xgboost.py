    ...
    with get_cursor() as cur:
        query = f"""
            SELECT
                ...
                COALESCE(ti.atr_14, 2.0) AS atr_pct, COALESCE(ti.volume_surge_ratio, 1.0) AS volume_ratio_20d,
                -- 등급 변경 이력
            ...
            LEFT JOIN LATERAL (
                SELECT section_a_technical, section_b_flow, section_c_macro,
                       rsi_14, macd_histogram, bb_width, COALESCE(atr_14, 2.0) AS atr_pct, COALESCE(volume_surge_ratio, 1.0) AS volume_ratio_20d
