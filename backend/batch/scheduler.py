        ...
        with get_cursor() as cur:
            ...
            for row in cur.fetchall():
                sig = {
                    ...
                    "pnl_pct": float(row.get("realized_pct") or 0),
                    "reason": row.get("sell_reason", row.get("signal_type", "SELL")),
                    "shares": int(row.get("shares") or 0),
                    "holding_days": int(row.get("holding_days") or 0),
                }
                if row.get("signal_type") == 'STOP_LOSS' and float(row.get("realized_pct") or 0) < -15:
    ...
    try:
        with get_cursor() as cur:
            ...
            if row:
                ...
                best_pnl = float(row.get("realized_pct", 0) or 0)
            ...
            if row:
                ...
                worst_pnl = float(row.get("realized_pct", 0) or 0)
    except Exception as e:
