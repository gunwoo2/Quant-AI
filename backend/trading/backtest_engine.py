"""
backtest_engine.py — 풀 백테스트 엔진 v3.1
...
v3.0 → v3.1 변경:
  ① Gap Down 감지: 전일종가 대비 시가 -7% 이상 → 즉시 매도
  ② Survivorship Bias 경고: 가격 데이터 끊긴 종목 강제 청산
  ③ Adaptive Threshold 연동: signal_generator v3.1
  ④ Stock Split 이상치 감지: 전일 대비 ±50% 변동 → 경고
"""
import numpy as np
import pandas as pd
...
class BacktestEngine:
    ...
    def run(self, start_date: date, end_date: date) -> BacktestResult:
        ...
        for i, today in enumerate(all_dates):
            ...
            for ticker, pos in list(self.positions.items()):
                ...
                # v3.1: Survivorship Bias — 가격 데이터 없음 → 강제 청산
                if price is None:
                    # 상장폐지 또는 데이터 끊김
                    print(f"  ⚠️ [SURVIVORSHIP] {ticker}: 가격 데이터 없음 ({today}) → 강제 청산")
                    sells.append((ticker, "DELISTED_OR_MISSING"))
                    continue



                # v3.1: Gap-Down 감지 (전일종가 대비 시가 -7% 이상)
                prev_close = self._get_price(ticker, today - timedelta(days=1))
                today_open = self._get_price(ticker, today, "open")
                if prev_close and today_open and prev_close > 0:
                    gap_pct = (today_open - prev_close) / prev_close
                    if gap_pct < -0.07:  # -7% 이상 갭다운
                        sells.append((ticker, f"GAP_DOWN({gap_pct:.1%})"))
                        continue  # 다른 조건 체크 불필요

                # 최고가 업데이트
                if price > pos.highest_price:
                    pos.highest_price = price

                # v3.1: Stock Split 이상치 감지 (전일 대비 ±50%)
                prev_c = self._get_price(ticker, today - timedelta(days=1))
                if prev_c and price and prev_c > 0:
                    change_pct = abs(price - prev_c) / prev_c
                    if change_pct > 0.5:
                        print(f"  ⚠️ [SPLIT?] {ticker}: {prev_c:.2f} → {price:.2f} ({change_pct:.0%} 변동)")
    ...
    def _execute_weekly_rebalance(self, d: date, regime: str):
        """주간 리밸런싱 — 새 매수 시그널 확인 및 실행 (v3.1: adaptive)"""
        total_val = self._portfolio_value(d)

        # v3.1: 시장 전체 점수 분포 계산 → adaptive threshold
        all_scores = []
        for t in self.stock_info:
            sd = self._get_score(t, d)
            if sd:
                all_scores.append(sd["final_score"])
        if len(all_scores) >= 10:
            import numpy as _np
            self._adaptive_thresholds = {
                "buy_threshold": max(self.cfg.buy_score_min, float(_np.mean(all_scores) + _np.std(all_scores))),
                "buy_l3_min": max(50.0, float(_np.mean(all_scores) * 0.9)),
            }
        ...
        for ticker, info in self.stock_info.items():
            ...
            # v3.1: Adaptive Threshold
            if not hasattr(self, '_adaptive_thresholds'):
                self._adaptive_thresholds = {"buy_threshold": self.cfg.buy_score_min, "buy_l3_min": self.cfg.buy_l3_min}
            if final_score < self._adaptive_thresholds.get("buy_threshold", self.cfg.buy_score_min):
                continue
            if l3_score < self._adaptive_thresholds.get("buy_l3_min", self.cfg.buy_l3_min):
                continue
