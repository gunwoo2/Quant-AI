...
def notify_morning_briefing(calc_date: date, regime: str, regime_detail: dict,
                            watchlist: list = None, positions_count: int = 0,
                            cash_pct: float = 0, earnings_today: list = None):
    """☀️ 모닝 브리핑 → MORNING 채널 (출근 도장)"""
    regime_emoji = {"BULL": "🟢", "NEUTRAL": "🟡", "BEAR": "🔴", "CRISIS": "🚨"}.get(regime, "⚪")
    ...
    fields = [
        ...
        {"name": "VIX", "value": f"{vix:.1f}" if vix else "N/A", "inline": True},
        {"name": "보유 종목", "value": f"{positions_count}개", "inline": True},
        {"name": "현금 비율", "value": f"{cash_pct:.1f}%", "inline": True},
    ]

    desc_lines = []
    if watchlist:
        desc_lines.append("**📋 주목 종목**")
        for w in watchlist[:5]:
            desc_lines.append(f"  {w['ticker']} ({w.get('score', 0):.0f}점) — {w.get('reason', '')}")
    if earnings_today:
        desc_lines.append("\n**📅 오늘 어닝콜**")
    ...
    embeds = [{
        ...
        "description": "\n".join(desc_lines) if desc_lines else "오늘의 시장 현황입니다.",
        "color": 0xf59e0b,
        "fields": fields,
        "footer": {"text": "QUANT AI v3.3 모닝 브리핑"},
    }]

    # MORNING 채널 (없으면 REPORT fallback)
    _send_discord(embeds=embeds, signal_type="MORNING")
    print(f"[NOTIFY] ☀️ 모닝 브리핑 → MORNING 채널")


def notify_weekly_report(calc_date: date, week_return: float, total_value: float,
                         top_winners: list = None, worst_losers: list = None,
                         trade_summary: dict = None):
    """🏆 주간 성과 리포트 → REPORT 채널 (성적표)"""
    fields = [
        {"name": "주간 수익률", "value": f"{week_return:+.2f}%", "inline": True},
        {"name": "총 자산", "value": f"${total_value:,.0f}", "inline": True},
    ]
    if trade_summary:
        fields.append({"name": "매수", "value": f"{trade_summary.get('buy', 0)}건", "inline": True})
        fields.append({"name": "매도", "value": f"{trade_summary.get('sell', 0)}건", "inline": True})
        fields.append({"name": "회전율", "value": f"{trade_summary.get('turnover', 0):.1f}%", "inline": True})

    desc_lines = []
    if top_winners:
        desc_lines.append("**🏆 Top Winners**")
        for w in top_winners[:3]:
            desc_lines.append(f"  {w['ticker']}  {w['pnl_pct']:+.1f}%  (${w.get('pnl_dollar', 0):+,.0f})")
    if worst_losers:
        desc_lines.append("\n**💀 Worst Losers**")
        for l in worst_losers[:3]:
            desc_lines.append(f"  {l['ticker']}  {l['pnl_pct']:+.1f}%  (${l.get('pnl_dollar', 0):+,.0f})")
...
def notify_regime_change(calc_date: date, old_regime: str, new_regime: str, detail: str = ""):
    """🚨 국면 전환 알림 → ALERT 채널"""
    emoji_map = {"BULL": "🟢", "NEUTRAL": "🟡", "BEAR": "🔴", "CRISIS": "🚨"}
    embeds = [{
        "title": f"⚡ 시장 국면 전환",
        "description": (
            f"{emoji_map.get(old_regime, '⚪')} **{old_regime}** → "
            f"{emoji_map.get(new_regime, '⚪')} **{new_regime}**\n\n"
            f"DynamicConfig가 자동으로 파라미터를 조정합니다.\n"
            f"{detail}"
        ),
        "color": 0xff6600,
        "fields": [
            {"name": "일자", "value": str(calc_date), "inline": True},
            {"name": "이전", "value": old_regime, "inline": True},
            {"name": "현재", "value": new_regime, "inline": True},
        ],
        "footer": {"text": "QUANT AI v3.3 국면 전환 알림"},
    }]

    _send_discord(embeds=embeds, signal_type="ALERT")
    print(f"[NOTIFY] ⚡ 국면 전환: {old_regime} → {new_regime}")


def notify_risk_warning(calc_date: date, dd_mode: str, drawdown_pct: float,
                        cb_level: str = "CLEAR", details: str = ""):
    """📐 리스크 경고 → ALERT 채널 (리스크 계기판)"""
    severity_color = {
        "NORMAL": 0x22c55e, "CAUTION": 0xf59e0b,
        "WARNING": 0xf97316, "DANGER": 0xef4444, "EMERGENCY": 0x7f1d1d,
    }
    embeds = [{
        "title": f"📐 리스크 경고 — {calc_date}",
        "color": severity_color.get(dd_mode, 0xff0000),
        "fields": [
            {"name": "DD 모드", "value": f"**{dd_mode}**", "inline": True},
            {"name": "낙폭", "value": f"{drawdown_pct:.1f}%", "inline": True},
            {"name": "CB 상태", "value": cb_level, "inline": True},
        ],
        "description": details if details else f"드로다운 {dd_mode} 단계 진입. 매수 제한이 적용됩니다.",
        "footer": {"text": "QUANT AI v3.3 리스크 모니터"},
    }]

    _send_discord(embeds=embeds, signal_type="ALERT")
    print(f"[NOTIFY] 📐 리스크 경고: DD={dd_mode} ({drawdown_pct:.1f}%)")
