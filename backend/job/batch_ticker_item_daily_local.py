...
# ─────────────────────────────────────────────────────
# DB 설정 — secret_info/config.py 에서 읽기 (로컬 전용)
# ─────────────────────────────────────────────────────
...
def run_daily_integrated_batch():
    ...
    try:
        ...
        # ── 로컬 테스트용: 특정 티커만 실행하려면 아래 리스트에 추가 ──
        # 빈 리스트([])로 두면 ticker_header 전체 실행
        TEST_TICKERS = ["AAPL", "MSFT", "GOOGL", "AVGO"]   # 예: ["AAPL", "MSFT", "GOOGL"]

        if TEST_TICKERS:
            cur.execute(
                "SELECT ticker FROM ticker_header WHERE ticker = ANY(%s) ORDER BY ticker",
                (TEST_TICKERS,)
            )
        else:
